...
from api.models import BLASTpResult
...
class BLASTpService:
    ...
    def validate_fasta_protein_file(self, file_path: Path) -> bool:
    ...
    def download_db(self, db_name: str, db_path: Path):
        """Download the requested BLAST database"""
        self.logger.info("Downloading BLAST database: %s", db_name)
        db_path.mkdir(exist_ok=True)
        self.logger.info("Moving to BLAST database path: %s", db_path)
        cwd = os.getcwd()
        os.chdir(db_path)
    ...
    def run_blastp_search(
        self, fasta_fpath: Path, evalue: float = DEFAULT_EVALUE, max_target_seqs: int = DEFAULT_MAX_TARGET_SEQS, outfmt: str = DEFAULT_OUTFMT
    ) -> Tuple[bool, str, BLASTpResult]:
        output_fpath = self.output_path.joinpath("blastp_output.txt")
        self.validate_fasta_protein_file(fasta_fpath)
        ...
        self.logger.info("DB path: %s", self.db_path)

        cmd = [
            "blastp",
            "--query", str(fasta_fpath),
            "--db", str(self.db_path),
            "--evalue", str(evalue),
            "--outfmt", outfmt,
        ...
        try:
            ...
            self.logger.info("BLASTp output file content: %s", output_fpath.read_text(encoding="utf-8"))
            # Parse results
    ...
    def _parse_blastp_table(self, output_file: Path, header: str=DEFAULT_HEADER) -> BLASTpResult:
        try:
